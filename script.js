...
function renderResult(student) {
  ...
  subjects.forEach(sub => {
    ...
    const rawWritten = student[sub.written];
    const rawOral = student[sub.oral];

    const obtainedWritten = safeNum(rawWritten);
    const obtainedOral = safeNum(rawOral);
    ...
    const writtenDisplay = (!fmStructure.written || rawWritten === undefined || rawWritten === null || rawWritten === '' || Number(rawWritten) === 0) ? '-' : obtainedWritten;
    const oralDisplay = (!fmStructure.oral || rawOral === undefined || rawOral === null || rawOral === '' || Number(rawOral) === 0) ? '-' : obtainedOral;
  ...
  resultElem.innerHTML = `<b>${hasFailed ? 'SUPPL.' : 'Pass'}</b>`;
